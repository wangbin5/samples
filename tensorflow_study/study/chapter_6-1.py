...
def inference(images):
    ...
    with tf.name_scope('cov5') as scope:
        ...
        pool5 = tf.nn.max_pool(conv5,ksize=[1,3,3,1],strides=[1,2,2,1],padding="VALID",name="pool1")
...
def time_tensorflow_run(session,target,info_string):
    ...
    sd = math.sqrt(vr)
    print('%s : %s accross %d steps,%.3f +/- %.3f sec /batch '%(datetime.now(),info_string,num_batches,mn,sd))
